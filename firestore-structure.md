...
```javascript
{
  ...
  active: boolean                 // Indica si el perfil está activo
  popularity: number              // Popularidad del perfil
}
```
...
```javascript
{
  ...
  passwordHash: string,           // Hash de la contraseña del usuario (Firebase Auth)
